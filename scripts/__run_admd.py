...
# TODO the start of the future Brain
...
def start_rp_client(rp_client):
    logger.info("Found RP Client: {}".format(rp_client.__class__))
    logger.info("Running?: {}".format(rp_client.running))
    if not rp_client.running:
        rp_client.start()
        logger.info("Running?: {}".format(rp_client.running))


def queue_tasks(project, tasks, rp_client, wait=False, batchsize=9999999, sleeptime=5):
...
def pythontask_callback(pythontask, scheduler, caller='__run_admd'):
    #pass
    logger.info("PythonTask Callback: Caller- {}".format(caller))
    logger.info("Project: {}".format(scheduler.project))
    output_stored = pythontask.output_stored
    supercheck = scheduler.project.tasks._set.load(pythontask.__uuid__, force_load=True).output_stored
    logger.info("Checks of output stored:")
    logger.info("output_stored: {}".format(output_stored))
    logger.info("supercheck: {}".format(supercheck))
    if any([output_stored, supercheck]):
        logger.info("Not doing callback, output already stored")
    else:
        outpath = os.path.expandvars(
            ...
            pythontask.__dict__['post'][1].target.url.replace(
              'project:///',
              '$ADMD_DATA/projects/{}/'.format(scheduler.project.name))
        )

        #print(pythontask.__dict__['post'])
        try:
            logger.info(pformat(list(scheduler.project.models._set._document.find())))
            logger.info("DOING PythonTask Callback with pythontask: {}".format(pythontask))
            logger.info("path to output json: {}".format(outpath))
...
def check_trajectory_minlength(project, minlength, n_steps=None, n_run=None, trajectories=[],
...
def model_task(project, modeller, margs, trajectories=None, min_trajlength=0,
               resource_requirements=None, taskenv=None, rp_client=None):
    ...
    if trajectories is None:
        trajectories = filter(lambda tj: tj.length >= min_trajlength, project.trajectories)
...
#
def strategy_function(project, engine, n_run, n_ext, n_steps, round_n,
                   ...
                   sfkwargs=dict(), rp_client=None, startontraj=0,
                   **kwargs):
    ...
    if rp_client:
        ...
        pre_cmds = gpu_cmds + ['module load python', 'which python',
                'echo \"   >>>  TIMER Task start \"`date +%s.%3N`']
        post_cmd = ['module unload python',
        ...
        resource_requirements = {'resource_name': project._current_configuration.resource_name,
       ...
       #     return margs
        ...
        def update_margs(_margs, round_n=None):
            ...
            if round_n:
                ...
                _margs = _margs[str(max(filter(lambda i: i <= round_n, map(lambda i: int(i), _margs))))]
                logger.info(pformat(_margs))
            # How many rounds are done?
    ...
    # FIXME need to incorporate this functionality
    #       into CUs with RP
    ...
    def all_done(mtask):
        ...
        if mtask:
            ...
            if mtask.is_done():
                ...
                logger.info("returning after wait")
                return False
...
def init_project(p_name, sys_name=None, m_freq=None, p_freq=None,
                 platform=None, reinitialize=False):#, dblocation=None):
